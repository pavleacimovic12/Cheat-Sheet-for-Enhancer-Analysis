...
print(f"DEBUG: base_metadata shape: {base_metadata.shape if not base_metadata.empty else 'Empty'}")
print(f"DEBUG: base_metadata columns: {list(base_metadata.columns) if not base_metadata.empty else 'None'}")
if not base_metadata.empty:
    print(f"DEBUG: Sample cargo values: {base_metadata['cargo'].unique()[:5]}")
    print(f"DEBUG: Sample experiment values: {base_metadata['experiment'].unique()[:5]}")
    print(f"DEBUG: Sample gc_delivered values: {base_metadata['gc_delivered'].unique()[:5]}")
...
selected_gc_delivered = st.sidebar.selectbox(
    ...
    help="Filter by GC delivered amount"
)
...
# Debug output
st.write(f"🔍 Applied filters: {len(filtered_enhancer_ids)} enhancers match criteria")
if len(filtered_enhancer_ids) > 0:
    st.write(f"📊 Filtered enhancers: {', '.join(filtered_enhancer_ids[:5])}" + ("..." if len(filtered_enhancer_ids) > 5 else ""))

# Individual enhancer analysis
if filtered_enhancer_ids:
            ...
            if imaging_urls:
                ...
                if len(all_viewers) > 1:
                    ...
                    for tab, (title, url) in zip(tabs, all_viewers):
                        with tab:
                            components.iframe(url, width=None, height=700)
                else:
