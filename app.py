...
st.write(f"DEBUG: base_metadata shape: {base_metadata.shape if not base_metadata.empty else 'Empty'}")
st.write(f"DEBUG: base_metadata columns: {list(base_metadata.columns) if not base_metadata.empty else 'None'}")
if not base_metadata.empty:
    st.write(f"DEBUG: Sample cargo values: {base_metadata['cargo'].unique()[:5]}")
    st.write(f"DEBUG: Sample experiment values: {base_metadata['experiment'].unique()[:5]}")
    st.write(f"DEBUG: Sample gc_delivered values: {base_metadata['gc_delivered'].unique()[:5]}")
